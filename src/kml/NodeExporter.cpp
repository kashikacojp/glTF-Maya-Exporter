...
namespace kml
{
	...
	static
	std::string GetIndentString(int level)
	{
		std::stringstream ss;
	...
	static
	void ExportMesh(FILE* fp, const std::string & path, int level, const std::shared_ptr<Mesh>& mesh)
	{
	...
	static
	void ExportNode(FILE* fp, const std::string & path, int level, const std::shared_ptr<Node>& node)
	{
		std::string strIndent = GetIndentString(level);
