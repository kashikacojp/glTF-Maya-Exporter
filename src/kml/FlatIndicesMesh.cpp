...
{
	static
	bool FixToReferenced(std::shared_ptr<Mesh>& mesh)
	{
		{
	...
	static
	bool NeedRecalcNormals(const std::shared_ptr<Mesh>& mesh)
	{
		if (mesh->normals.size() == 0)
		{
			return true;
	...
	static
	bool CheckNormals(std::shared_ptr<Mesh>& mesh)
	{
	...
	static
	bool FixIndices(std::shared_ptr<Mesh>& mesh)
	{
#if 0
	...
	static
	bool ShortenNormal(std::shared_ptr<Mesh>& mesh)
	{
		for (size_t i = 0; i < mesh->normals.size(); i++)
		{
