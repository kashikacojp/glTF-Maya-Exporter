...
namespace kml
{
	...
	static
	bool NeedRecalc(const std::shared_ptr<Mesh>& mesh)
	{
	...
	static
	bool IsFaceVaryingNormal(const std::shared_ptr<Mesh>& mesh, float E)
	{
		if (mesh->normals.size() != 0)
		{
	...
	static
	bool CalculateNormalsMeshFaceVaring(std::shared_ptr<Mesh>& mesh, float E)
	{
		std::vector<glm::vec3> nf(mesh->facenums.size());
		for (size_t i = 0; i < nf.size(); i++)
		{
	...
	static
	bool CalculateNormalsMeshVertexVaring(std::shared_ptr<Mesh>& mesh)
	{
		if (!NeedRecalc(mesh))
		{
			return true;
